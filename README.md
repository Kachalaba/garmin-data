...
> Щоденний health-звіт: годинник Garmin пише сон, пульс, стрес → комп'ютер щоранку сам рахує, як ти відновився, та надсилає висновок у Notion простою мовою.
...
Цей проєкт робить три прості речі:
...
2. **Щодня сам рахує за тебе** порівняння з твоєю особистою нормою за останні 1–2 місяці. Наприклад: "сьогодні пульс у спокої на 2 удари вище середнього — це нормальний розкид" або "вже другий день поспіль — схоже, починаєш хворіти".
3. **Пише короткий звіт у Notion** людською мовою: "відновився добре, можна тренуватись" / "недоспав, краще спокійний день" / "увага: 3 дні поспіль знижений HRV".
...
|--------|-------|--------------|
| **RHR** (resting heart rate) | Пульс у спокої — скільки ударів за хвилину, коли ти лежиш і нічого не робиш | Ранковий RHR зазвичай стабільний. Якщо раптом виріс на 5+ ударів — організм або бореться з чимось (хвороба), або перевтомлений |
| **HRV** (heart rate variability) | Варіативність пульсу — наскільки нерівномірно бʼється серце між ударами (за ніч) | Високе HRV = ти добре відновився. Низьке = стрес, недосип, або застуда. Цифра дуже персональна — порівнюй себе з собою, а не з іншими |
| **SpO₂** | Наскільки кров насичена киснем (у %) | У здорової людини ~96–99%. Різке падіння може бути ознакою респіраторного захворювання |
...
| **ACWR** | Співвідношення навантаження за 7 днів до 28 днів | Якщо різко стрибнув (наприклад, 1.5+) — ризик травми у кілька разів вище. Золота зона 0.8–1.3 |
...
Скрипт `garmy_sync.py` синхронізує дані здоров'я з Garmin Connect у локальну SQLite-базу. Пакет `analytics/` обчислює похідні метрики (HRV baseline, RHR anomaly, погоду для активностей, прогностичні ризики) у додаткові таблиці тієї ж БД. Далі — дві scheduled Claude-рутини читають усе разом і дотягують метрики форми з Intervals.icu наживо, формуючи Notion-дайджести. Grafana-дашборд у `grafana/` дає always-on візуальний шар над тими ж даними.
...
> 🙂 **Простими словами:** Garmin щоночі міряє, наскільки нерівномірно бʼється твоє серце (HRV). Цей показник дуже персональний — у одного норма 35 мс, у іншого 80 мс. Скрипт дивиться на твої останні 60 ночей, рахує твій особистий "коридор нормальності" і каже: сьогоднішня ніч у нормі, вище норми (добре відновився, є резерв) чи нижче (недоспав / втомлений / захворюєш).
...
### ❤️ `rhr_anomaly` — раннє попередження хвороби за пульсом

> 🙂 **Простими словами:** коли ти хворієш, організм починає воювати з інфекцією ще до того, як з'являються симптоми (нежить, горло). Одна з перших ознак — пульс у спокої росте на кілька ударів. Стенфордський університет довів на даних 32 носіїв Fitbit/Garmin: можна впіймати застуду чи COVID **за 2–3 дні до симптомів**. Цей скрипт порівнює твій вчорашній ранковий пульс із середнім за 28 днів і попереджає, якщо щось нетипове — особливо якщо **2 дні поспіль** (одноразовий стрибок зазвичай нічого не означає).
...
### 🎲 `risk_scores` — шість щоденних "передбачень"

> 🙂 **Простими словами:** це свого роду автоматичний "лікар", що дивиться на всі попередні дані й щодня дає шість коротких відповідей на питання: "чи не захворію найближчими днями?", "чи не перетренувався?", "чи досипаю?", "чи звикає тіло до спеки?", "чи не падає моя форма кілька тижнів поспіль?".

Нижче — розшифровка кожного показника. Кожен має посилання на наукову роботу, на якій побудована формула:

**Illness Risk Score** (0–100) — **ймовірність захворіти у найближчі 2–3 дні.** Об'єднує чотири сигнали: виріс пульс, впав HRV, зросла частота дихання уві сні, впав SpO₂. Якщо три з чотирьох "миготять" одночасно — це дуже схоже на початок інфекції. Ґрунтується на роботі Stanford/Snyder lab під час COVID. Рівні: LOW (норма) / SLIGHT (дрібний сигнал) / ELEVATED (варто придивитися) / HIGH (дуже ймовірно починаєш хворіти).

**ACWR** — **навантаження за тиждень проти навантаження за місяць.** Якщо раптом різко натренувався (ACWR > 1.5) — травма ймовірніша у 2–4 рази (Gabbett 2016, класика спортивної науки). "Золота зона" 0.8–1.3 — це де форма росте з мінімальним ризиком. Нижче 0.8 — ти мало тренуєшся й форма падає.
