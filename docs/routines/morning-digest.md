...
- Якщо persistent=1 — виділити як ⚠️ "2+ дні поспіль нетиповий rHR; перевір контекст і самопочуття"
...
**🫀 Чи є нетипові фізіологічні відхилення?** [🟢/🟡/🔴/❔] — [illness_risk_score]/100 ([illness_risk_level])
[LOW → "доступні метрики близькі до персонального baseline"]
[MODERATE → "є невеликі зрушення (назвати які з illness_risk_drivers), перевір контекст і самопочуття"]
[HIGH → "кілька метрик одночасно відхилилися від baseline; знизити навантаження і спостерігати за самопочуттям"]
...
[DANGER_ZONE + chronic_load_28d ≥ 20 → "тижневе навантаження різко вище місячної бази; розглянути розвантаження"]
...
⚠️ Підозрілий max HR — **не** називати артефактом без перевірки. Спершу звір із персональним максимумом ЧСС (`{{MAX_HR}}`): пік на рівні 90–95% максу на фінальному важкому відрізку — норма. Якщо все одно сумнівно — витягни посекундний потік із FIT-файлу та подивись форму кривої:
```
{{PYTHON_PATH}} -m pip install fitparse --quiet   # якщо ще нема
# intervals-icu download_fit_file(activity_id, output_path="/tmp/act.fit")
# далі fitparse: get_messages('record') → (timestamp, heart_rate), дискретизація 1с
```
Плавний набір + плато на піку = **реальний** пульс; одиночний зубець вгору-вниз або рваний шум = похибка датчика. Локальна таблиця `timeseries` зберігає пульс лише у денному ~2-хв розрізі — для піків окремих відрізків вона надто груба, тому й потрібен FIT.
...
| `{{MAX_HR}}` | Твій максимальний пульс (для перевірки підозрілих піків у розборі тренування) | `~204 на суші, оптика Garmin` |
